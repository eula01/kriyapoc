 ...
 */
export async function fetchCompanyByCRN(
  crn: string
  ...
  try {
    ...
    headers.append(
      "Authorization",
      "Basic M2RmZmZjMDUtMGZmYy00NWMzLTg4YzUtZTlkNzg4YzQwMWVkOg=="
    );
    ...
    if (!response.ok) {
      ...
      console.error(
        `❌ [Server Action] Companies House API error ${errorStatus}`
      );
    ...
    console.log(
      `👻👻👻 [Server Action] Companies House Data: ${JSON.stringify(
        data,
        null,
        2
      )}`
    ...
    if (data.company_name) {
      ...
      const cleanedName = data.company_name
        .replace(
          /\s+(LIMITED|LTD|PLC|LLC|INC|INCORPORATED|CORPORATION|CORP)\.?$/i,
          ""
        )
        .trim();
